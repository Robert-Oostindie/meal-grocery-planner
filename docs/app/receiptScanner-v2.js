// ============================================================
// RECEIPT SCANNER MODULE (receiptScanner.js) — v2
//
// v2: unit-price + qty per item, receipt subtotal cross-check
// (misread-digit warning), higher JPEG quality for OCR accuracy.
...
let receiptFiles = [];
let parsedItems = [];      // [{ name, price, qty, include }]
let receiptSubtotal = null; // printed subtotal from the receipt, for cross-checking
...
export async function scanReceipt() {
    ...
    try {
        ...
        receiptSubtotal = (typeof data.receiptSubtotal === "number") ? data.receiptSubtotal : null;
...
function renderReceiptReview() {
    ...
    // ── Misread-digit cross-check ──────────────────────────
    // Compare sum of (unit price x qty) against the printed
    // subtotal. A gap means a digit was probably misread
    // (6/8, 5/8, 3/8 are common on dot-matrix receipts).
    if (receiptSubtotal != null) {
        const itemsTotal = parsedItems.reduce(
            (sum, it) => sum + (Number(it.price) || 0) * (it.qty || 1), 0);
        const gap = Math.abs(itemsTotal - receiptSubtotal);

        if (gap > 0.25) {
            const warn = document.createElement("div");
            warn.style.cssText = "background:#fef3c7; border:1px solid #fcd34d; border-radius:8px; padding:0.5rem 0.75rem; margin-bottom:0.6rem; font-size:0.85rem; color:#92400e;";
            warn.textContent = `\u26A0\uFE0F Extracted items total $${itemsTotal.toFixed(2)} but the receipt subtotal is $${receiptSubtotal.toFixed(2)} \u2014 a digit may have been misread. Double-check the prices below against your receipt.`;
    ...
    parsedItems.forEach((item, idx) => {
        ...
        // Show quantity purchased (price shown/saved is per unit)
        if ((item.qty || 1) > 1) {
            const qtyLabel = document.createElement("span");
            qtyLabel.textContent = `\u00D7${item.qty}`;
            qtyLabel.title = "Quantity purchased \u2014 the price shown is per unit";
            qtyLabel.style.cssText = "font-size:0.8rem; color:#6b7280; flex-shrink:0; min-width:24px;";
